...
这顺便就解答了第二个问题：因为有预设好的 SBP Signature，所以，某一层算子只要有输入的 SBP，OneFlow 就可以根据 SBP Signature 推导出该层算子输出的 SBP。而上次算子的输出，又是下层算子的输入，这样，就确定了下层算子输入的 SBP，然后又可以根据 SBP Signature 确定下一层输出的 SBP……
这样不断推导、传播。因此通常情况下，用户是不需要为每层网络都设置输入的 SBP。而只又当最初输入层，或者需要强制指定某层的 SBP 时，才需要显式指定。
