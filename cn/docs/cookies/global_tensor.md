...
全局视角（Global View）编程提供了单程序单数据（SPSD）的编程视角。和 SPMD 编程不同的是，数据从编程接口层面看也是单一的了。数据是同一个逻辑数据，其实很自然的，当我们把一个单进程程序扩展成并行执行时，一个单进程数据被扩展成多进程数据，但是多个进程上的这些数据其实可以被看做是同一个逻辑数据，这个逻辑数据在 OneFlow 中叫 Global Tensor。

编程时，Global Tensor 让用户可以用 SPSD 接口来编程，按照单机单设备的逻辑视角来编程。然后 OneFlow 框架内部自动的转换成物理的 SPMD/MPMD 方式来做并行/分布式执行。
