...
比如：`ids = np.array([488, 333, 220, 18, 568, 508], dtype=np.int64)`，`table_ids = np.array([0, 1, 2, 0, 1, 2])`，然后调用： `embedding_lookup(ids, table_ids)`，则表示在第 0 个表中查询 `488, 18`，第 1 个表中查询 `333, 568`，第 2 个表中查询 `220, 508` 对应的特征向量。
