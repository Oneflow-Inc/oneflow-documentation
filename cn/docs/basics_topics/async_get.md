...
通常，我们训练模型的过程都是同步的，接下来我们展开说明同步和异步的概念，以及在 OneFlow 中异步执行的优势。
...
在异步训练中，相当于开启了多线程模式，某个 step 不必等上一个 step 的作业结束，而是可以提前进行数据预处理和加载过程。
...
* 准备的回调函数，会被 OneFlow 调用，并将定义作业函数的返回值作为参数传递给回调函数

以上工作的前三步由 OneFlow 的用户完成，最后一步由 OneFlow 框架完成。
