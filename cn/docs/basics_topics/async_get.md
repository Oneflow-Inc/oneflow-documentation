...
在 OneFlow 中，通常将用 `@flow.global_function` 装饰器修饰的函数定义为作业函数(Job)，此任务可能是训练、验证或预测任务。可以通过 `get()` 方法和 `async_get()` 方法来获取作业函数被执行后的返回对象/结果，`get` 和 `async_get` 则分别对应表示同步和异步获取结果。
...
通过以上对比可知，在 OneFlow 中使用异步执行作业，有效利用了计算机资源，尤其是在数据集规模巨大的情况下，**开启异步执行能有效缩短数据的加载和准备时间，加快模型训练**。
...
调用作业函数，得到一个 OneFlow 对象，该对象的 `get` 方法，可以同步方式结果。
...
那么，我们可以使用以下代码，通过调用 `get` 方法，获取作业函数所返回的 loss ，并打印平均值。
...
因为 OneFlow 框架的特点，定义作业函数时所 `return` 的对象，在调用作业函数时并 **不是** 直接得到，而需要进一步调用 `get` （及下文介绍的 `async_get` ）方法获取。


## 异步获取结果

一般而言，采用异步方式获取训练结果的效率高于同步方式。
以下介绍如何通过调用作业函数的 `async_get` 方法，异步获取训练结果。

其基本步骤包括：

* 准备回调函数，在回调函数中实现处理作业函数的返回结果的逻辑

* 通过 async_get 方法注册回调

* OneFlow 在合适时机调用注册好的回调，并将作业函数的训练结果传递给该回调
...
其中的 result ，就是作业函数的返回值

比如，在以下的作业函数中，返回了 loss 。
...
返回了一个字典，分别存储了 labels 和 logits 两个对象。
...
### 注册回调函数
调用作业函数，会返回 `blob` 对象，调用该对象的 `async_get` 方法，可以注册我们实现好的回调函数。
...
### 同步获取多个返回结果
在本例中，作业函数返回一个 `list` ，我们通过同步方式获取 `list` 中 `labels` 与 `logits` ，并对上例中训练好的模型进行评估，输出准确率。
...
在以下的例子中，作业函数返回一个 `dict` ，我们展示了如何异步方式获取 `dict` 中的多个返回结果。
