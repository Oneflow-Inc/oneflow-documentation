...
The following complete code shows how to generate an OFRecord file and manually read the data by calling `OFRecord` interface generated by `protobuf`.
...
### Write OFRecord object to the file

In the following code, we simulate 3 samples, each sample is a `28*28` picture and contains the corresponding label.After the three samples are converted into OFRecord objects, they are stored in the file according to the OneFlow convention format.

The complete code：[ofrecord_to_string.py](../code/extended_topics/ofrecord_to_string.py)
...
In the following code, we will read `OFRecord` file generated in the above example. Call the `FromString` method to deserialize to obtain the `OFRecord` object, and finally displays the data：
