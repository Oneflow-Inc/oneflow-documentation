...
OneEmbedding是一个大规模Embedding的oneflow方案，可以解决大规模深度推荐系统的问题，它有以下特点：
    灵活的分层存储，支持将 Embedding table 放置在 GPU显存、CPU内存 、或者 SSD 上面，使用高速设备作为低速设备的缓存，实现速度与容量的兼顾。
    支持动态插入新特征ID。
...
# store_options是OneEmbedding中词表的存储配置选项，OneEmbedding提供了三种预置参数配置，分别是纯GPU存储配置，GPU CPU存储配置和GPU SSD存储配置，详情点击
...
"""
    ...
    physical_block_size：Embedding词表持久化存储中使用的physical block size，physical block size应为磁盘扇区大小，一般为 512，physical_block_size 默认值是512，一般不用配置，若底层硬件设备的磁盘扇区大小是4096，则需要设置为4096。
"""
# 使用make_device_mem_store_options获得纯GPU存储配置
def make_device_mem_store_options(persistent_path, capacity, size_factor=1, physical_block_size=512)

# 使用make_cached_host_mem_store_options获得GPU CPU存储配置，此时词表存储在CPU内存中，并使用GPU显存作为高速缓存
def make_cached_ssd_store_options(cache_budget_mb, persistent_path, capacity, size_factor=1,   physical_block_size=512)

# 使用make_cached_ssd_store_options获得GPU SSD存储配置，此时词表存储在SSD中，并使用GPU显存作为高速缓存
