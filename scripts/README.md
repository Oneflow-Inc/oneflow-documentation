# 如何使用 docs.config 和 mkcodes.py 进行文档的自动检查

## 基本介绍
docs.config 是一个配置文件，mkcodes.py 将根据其中的内容，查找仓库中指定的文档，并测试指定的代码块和验证输出。比如下面的这个例子，mkcodes.py 会在配置合适的情况下，运行代码块中的代码，并将下方的文本块和程序的输出进行比较验证。如果程序运行错误或者比较结果不一致，则会报错。
...
原则上，为了防止新建的文档忘记被收录，docs.config 记录了本仓库中所有的文档，且会在发现有未被记录的文档时**报错**。因此，即使文档内没有需要被测试的代码，也需要将文档的相对路径和名称在 docs.config 内进行配置。  
...
1. 首先，前往 scripts 文件夹，打开 docs.config ，在文档的结尾处新建一行，输入：
```
